...
volatile uint32_t* btn_gpio_data = XPAR_AXI_GPIO_1_BASEADDR;
volatile uint32_t* sw_gpio_data = XPAR_AXI_GPIO_2_BASEADDR;

uint32_t accumulator = 0;
uint16_t btn_status = 0;	// 1 is pressed, 0 is unpressed
...
int main()
{
	...
	while (1+1 != 3)
	{
		*led_gpio_data = accumulator;
		...
		if ((*btn_gpio_data == 8) && (btn_status == 0)) {
